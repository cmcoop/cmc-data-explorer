...
namespace cmc2.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> EditProfile(ManageMessageId? message)
        {
            ...
            {
                crntUserRole = crntUserRole + item;
            }
            var model = new IndexViewModel
            {
                ...
                CellPhone = Extensions.formatPhone(user.CellPhone),
                HomePhone = Extensions.formatPhone(user.HomePhone),
                EmergencyPhone = Extensions.formatPhone(user.EmergencyPhone),
                AddressFirst = user.AddressFirst,
                AddressSecond = user.AddressSecond,
                City = user.City,
                State = user.State,
                Zip = user.Zip,
                UserRole = crntUserRole,
                VolunteerHours = user.VolunteerHours,
                GroupName = context.Groups.Where(g => g.Id.Equals(user.GroupId)).Select(n => n.Name).
                                                    FirstOrDefault().ToString(),
                StatesList = statesList.Select(x => new SelectListItem()
                {
        ...
        public async Task<ActionResult> EditProfile([Bind(Exclude = "ProfileImage")]IndexViewModel editUser)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                user.CellPhone = Extensions.GetNumbers(editUser.CellPhone);
                user.HomePhone = Extensions.GetNumbers(editUser.HomePhone);
                user.EmergencyPhone = Extensions.GetNumbers(editUser.EmergencyPhone);
                user.AddressFirst = editUser.AddressFirst;
                user.AddressSecond = editUser.AddressSecond;
                user.City = editUser.City;
                user.State = editUser.State;
                user.Zip = editUser.Zip;
                user.ModifiedBy = editUser.Id;
                user.ModifiedDate = DateTime.Now;


                editUser.StatesList = statesList.Select(x => new SelectListItem()
                {
                    Selected = x.Value.Equals(user.State),
                    Text = x.Text,
                    Value = x.Value
        ...
        public async Task<ActionResult> VerifyPhoneNumber(VerifyPhoneNumberViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                return RedirectToAction("EditProfile", new { Message = ManageMessageId.AddPhoneSuccess });
            }
            // If we got this far, something failed, redisplay form
        ...
        public async Task<ActionResult> RemovePhoneNumber()
        {
            ...
            if (!result.Succeeded)
            {
                return RedirectToAction("EditProfile", new { Message = ManageMessageId.Error });
            }
            var user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
            if (user != null)
            {
            ...
            return RedirectToAction("EditProfile", new { Message = ManageMessageId.RemovePhoneSuccess });
        }
        ...
        public async Task<ActionResult> SetPassword(SetPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    return RedirectToAction("EditProfile", new { Message = ManageMessageId.SetPasswordSuccess });
                }
                AddErrors(result);
            }
